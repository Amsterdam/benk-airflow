...
class Prepare(BaseDAG):
    ...
    def _prepare_task_operator(self, action_id: str):
        return self.Operator(
            ...
            arguments=["prepare_task", f"--catalogue={self.catalogue}", f"--task_name={action_id}"],
            **PrepareArgs,
        )
    ...
    def _init(self):
        ...
        leaf_nodes = [
            ...
            if task_id not in all_dependencies and task_id not in final_action_ids
        ]
        ...
        self._leaf_nodes = leaf_nodes if not final_action_ids else [tasks[action_id] for action_id in final_action_ids]
